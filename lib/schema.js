...
const SITE_NAME = 'SmartSoft Solutions';
...
export function organization({
    ...
    const schema = {
        ...
        address: {
            '@type': 'PostalAddress',
            streetAddress: '24 Ranchlands Bay NW',
    ...
    if (description) schema.description = description;
    if (email) schema.contactPoint[0].email = email;
...
export function localBusiness({
    ...
    return {
        ...
        '@id': url,
        name,
        description: description || 'CPA-supervised bookkeeping, accounting, payroll, and tax services for businesses in the USA and Canada.',
        url,
        telephone,
        openingHours,
        priceRange,
        ...
        areaServed: Array.isArray(areaServed)
            ? areaServed.map(c => ({ '@type': 'Country', name: c }))
            : { '@type': 'Country', name: areaServed },
        hasOfferCatalog: {
            '@type': 'OfferCatalog',
            name: 'Financial Services',
            itemListElement: [
                { '@type': 'Offer', itemOffered: { '@type': 'Service', name: 'Bookkeeping & Reconciliation' } },
                { '@type': 'Offer', itemOffered: { '@type': 'Service', name: 'CPA Supervised Accounting' } },
                { '@type': 'Offer', itemOffered: { '@type': 'Service', name: 'Payroll Processing' } },
                { '@type': 'Offer', itemOffered: { '@type': 'Service', name: 'Tax Preparation' } },
                { '@type': 'Offer', itemOffered: { '@type': 'Service', name: 'Financial Consulting' } },
            ],
        },
    };
}
...
export function article({
    ...
    const schema = {
        ...
        publisher: {
            ...
            logo: {
                '@type': 'ImageObject',
                url: `${SITE_URL}/logo.png`, // Assuming logo exists
            }
        },
        datePublished,
